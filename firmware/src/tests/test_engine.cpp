...
// goes through SafetyInterlocks guarded interfaces. CAN switching is explicit
// state-machine sequencing (all-CAN-OFF -> OFF-settle -> ON -> settle -> read).
// Every new test begins from a guaranteed all-outputs-OFF physical state.
// =============================================================================
...
ContinuityResult classifyContinuity(float baseline, float measured) {
  ...
  if (measured >= gCfg.continuityMinV && measured <= gCfg.continuityMaxV &&
      delta >= gCfg.continuityMinV) {
...
CrossResponseResult classifyCross(float delta) {
  return CrossResponseResult{ delta >= gCfg.crossResponseDeltaV, delta };
}
...
bool readChannel(Channels::AdcChannel ch, float& v) {
  if (!Channels::isValidDiagnosticChannel(ch)) return false;
  return AdcService::readNodeVolts(ch, v);
}

bool readPinNode(uint8_t pin, float& v) {
  return readChannel(Channels::pinToChannel(socketFor(gParams.mode), pin), v);
}
...
void markFocusStep() {
  ...
  if (r != nullptr) r->stepIndex = gStepIndex;
}

void stepSafeCheck() {
  if (isLoad(gParams.mode)) {
    if (gParams.mode == TestMode::LAMP_ISO12098 && !isApprovedLampPin(gParams.lampPin)) {
      fault(AbortReason::PRECONDITION); return;
    }
    if (gParams.mode == TestMode::AXLE_LIFT && !gParams.axleSafetyConfirmed) {
      fault(AbortReason::PRECONDITION); return;
    }
  ...
  if (isVoltage(gParams.mode)) {
    if (!SafetyInterlocks::applyMeasurementReference(1UL << TpicBit::K6_MASTER_GND)) {
...
void stepBaseline() {
  ...
  while (gChanIdx < list.count) {
    ...
    gBaseline[(uint8_t)list.ch[gChanIdx]] = v;
    ++gChanIdx;
  }
  ...
  for (uint8_t p = 1; p <= n && gRes.cableCount < kMaxPins12098; ++p) {
    ...
    r.pin = p; r.ch = ch;
    r.stepIndex = 0;
    r.baselineV = gBaseline[(uint8_t)ch];
    r.focusV = 0; r.processed = false;
  ...
  if (!SafetyInterlocks::applyCableTestOutput(1UL << Channels::tpicBitFor(out))) {
    fault(AbortReason::INTERLOCK_REJECTED); return;
  }
  markFocusStep();
...
void stepFocusRead() {
  ...
  if (r != nullptr) {
    r->focusV = v;
    r->continuity = classifyContinuity(r->baselineV, v);
    r->processed = true;
  }
  gChanIdx = 0;
...
void stepCrossScan() {
  ...
  while (gChanIdx < list.count) {
    ...
    if (ch != gFocusCh) {
      ...
      const float base = gBaseline[(uint8_t)ch];
      const float delta = v - base;
      const CrossResponseResult cr = classifyCross(delta);
      if (cr.isCoupled) {
        const Channels::SocketPin sp = Channels::channelToPin(ch);
        addShort(gFocusPin, sp.valid ? sp.pin : 0, base, v, cr.delta);
      }
    }
    ++gChanIdx;
  }
...
void stepSweep() {
  ...
  r.pin = gPinIdx; r.ch = ch; r.nodeV = v; r.valid = true;
  r.k6OffValid = false; r.pulseValid = false;

  if (s == Channels::Socket::ISO12098 && (gPinIdx == 1 || gPinIdx == 2)) {
    PulseMonitor::PulseEvidence ev;
    const PulseMonitor::PulseInput pi =
        (gPinIdx == 2) ? PulseMonitor::PulseInput::SAG : PulseMonitor::PulseInput::SOL;
...
void stepTermRead() {
  ...
  if (!okH || !okL) {
    SafetyInterlocks::clearCanSelection();
    fault(AbortReason::SERVICE_FAULT);
    return;
  }
  gRes.term.relay = termRelayFor(m);
  gRes.term.vhV = vh; gRes.term.vlV = vl; gRes.term.deltaV = vh - vl;
  gRes.term.valid = true;
  SafetyInterlocks::clearCanSelection();
...
void stepLoad() {
  if (!gLoadSampled) {
    ...
    gRes.load.shuntV = smp.shuntVolts; gRes.load.shuntValid = smp.shuntValid;
    gRes.load.currentA = smp.currentA; gRes.load.currentValid = smp.currentValid;
    gRes.load.busV = smp.busVolts;     gRes.load.busValid = smp.busValid;
    gLoadSampled = true;
    return;
  }
...
bool start(const TestStartParams& params) {
  ...
  gPinIdx = 1; gChanIdx = 0; gFocusPin = 0; gStepIndex = 0;
  gFocusCh = Channels::kNoChannel;
  gLoadSampled = false; gLoadStart = 0;
  gAbort = AbortReason::NONE;
...
bool isActive() {
  return gState != TestState::IDLE && gState != TestState::COMPLETE &&
         gState != TestState::ABORTED && gState != TestState::FAULT;
}
