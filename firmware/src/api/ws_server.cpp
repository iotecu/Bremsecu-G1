...
// Package 5 keeps engineering/pin values and raw node evidence explicit.
// =============================================================================
#include "ws_server.h"
#include <Arduino.h>
...
void onEvent(uint8_t num,WStype_t type,uint8_t*payload,size_t length){(void)payload;(void)length;if(type==WStype_CONNECTED){String j=deviceStatusJson();gWs.sendTXT(num,j);}}
...
void emitCableProgress(const TestEngine::TestResults&r,TestEngine::TestMode m,TestEngine::TestState st){
  ...
  while(g.emitted<completed){
    const auto&c=r.cable[g.emitted];
    ...
    s+=",\"focusPinV\":";s+=String(c.focusPinV,3);
    s+=",\"focusNodeV\":";s+=String(c.focusNodeV,3);
    s+=",\"conversion\":\"";s+=convStr(c.focusConversion);s+="\"";
    s+=",\"continuity\":\"";s+=contStr(c.continuity);
    s+="\",\"shorts\":[";
    bool f=true;
    for(uint8_t i=0;i<r.shortCount;++i){const auto&x=r.shorts[i];if(x.focusPin!=c.pin)continue;if(!f)s+=",";f=false;s+="{\"from\":";s+=String(x.focusPin);s+=",\"to\":";s+=String(x.coupledPin);s+=",\"deltaPinV\":";s+=String(x.deltaPinV,3);s+="}";}
    s+="] ,\"progress\":{\"completed\":";s+=String(g.emitted+1);s+=",\"total\":";s+=String(r.cableCount);s+="}";
    s+=",\"classificationFinal\":false}";send(s);++g.emitted;
  }
}

void emitCrossScan(const TestEngine::TestResults&r,TestEngine::TestMode m){
  while(g.shorts<r.shortCount){
    const auto&x=r.shorts[g.shorts];
    String s="{\"type\":\"cross_scan_update\",\"mode\":\"";s+=modeStr(m);
    s+="\",\"socket\":\"";s+=socketStr(m);
    s+="\",\"focusPin\":";s+=String(x.focusPin);
    s+=",\"scannedPin\":";s+=String(x.coupledPin);
    s+=",\"baselinePinV\":";s+=String(x.baselinePinV,3);
    s+=",\"measuredPinV\":";s+=String(x.measuredPinV,3);
    s+=",\"deltaPinV\":";s+=String(x.deltaPinV,3);
    s+=",\"baselineNodeV\":";s+=String(x.baselineNodeV,3);
    s+=",\"measuredNodeV\":";s+=String(x.measuredNodeV,3);
    s+=",\"isCoupled\":true,\"classificationFinal\":false}";send(s);++g.shorts;
  }
}
...
void emitCableCompleted(const TestEngine::TestResults&r,TestEngine::TestMode m){
  ...
  s+="\",\"mode\":\"";s+=modeStr(m);s+="\",\"testedPins\":";s+=String(r.cableCount);
  s+=",\"passCount\":";s+=String(pass);s+=",\"openCount\":";s+=String(open);s+=",\"indeterminateCount\":";s+=String(indet);s+=",\"shortCount\":";s+=String(r.shortCount);
  s+=",\"shorts\":[";for(uint8_t i=0;i<r.shortCount;++i){if(i)s+=",";const auto&x=r.shorts[i];s+="{\"from\":";s+=String(x.focusPin);s+=",\"to\":";s+=String(x.coupledPin);s+=",\"deltaPinV\":";s+=String(x.deltaPinV,3);s+="}";}
...
void emitVoltage(const TestEngine::TestResults&r,TestEngine::TestMode m){
  while(g.volt<r.voltCount){
    ...
    e+="\",\"pin\":";e+=String(v.pin);e+=",\"channelId\":";e+=String((unsigned)v.ch);
    e+=",\"nodeValue\":";e+=String(v.nodeV,3);e+=",\"nodeUnit\":\"V\"";
    ...
    e+=",\"classificationFinal\":false}";send(e);++g.volt;
  }
}

void emitLoad(const TestEngine::TestResults&r,TestEngine::TestMode m){if(r.load.shuntValid&&!g.loadValid){g.loadValid=true;send(loadCurrentJson(r));}if(r.load.onMs!=0&&r.load.onMs!=g.loadOnMs){g.loadOnMs=r.load.onMs;send(loadCurrentJson(r));}}

void emitTerm(const TestEngine::TestResults&r,TestEngine::TestMode m){
  if(isTerm(m)&&r.term.nodeValid&&!g.termValid){
    g.termValid=true;
    String s="{\"type\":\"termination_result\",\"mode\":\"";s+=modeStr(m);
    s+="\",\"canHNodeV\":";s+=String(r.term.canHNodeV,3);
    s+=",\"canLNodeV\":";s+=String(r.term.canLNodeV,3);
    s+=",\"deltaNodeV\":";s+=String(r.term.deltaNodeV,3);
    s+=",\"resistanceOhms\":";
    if(r.term.conversion==MeasurementConversion::ConversionStatus::OPEN_CIRCUIT)s+="null";else if(r.term.resistanceValid)s+=String(r.term.resistanceOhms,2);else s+="null";
    s+=",\"conversion\":\"";s+=convStr(r.term.conversion);s+="\"";
    s+=",\"valid\":";s+=r.term.resistanceValid?"true":"false";
    s+=",\"classificationFinal\":false}";send(s);
  }
}
...
void notifyRecordUpdated(const char*recordId,const char*testId){if(!gReady)return;String s="{\"type\":\"record_updated\",\"recordId\":\"";for(const char*p=recordId;*p;++p){if(*p=='"'||*p=='\\')s+='\\';s+=*p;}s+="\",\"testId\":\"";for(const char*p=testId;*p;++p){if(*p=='"'||*p=='\\')s+='\\';s+=*p;}s+="\"}";gWs.broadcastTXT(s);}
} // namespace WsServer
